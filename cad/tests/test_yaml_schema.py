...
def test_yaml_schema_required_keys():
    ...
    assert data["locked_dimensions"]["permaproto_quarter"]["board_x_mm"] == 50.8
    assert data["locked_dimensions"]["permaproto_quarter"]["board_y_mm"] == 43.0
    assert data["locked_dimensions"]["permaproto_quarter"]["board_z_mm"] == 1.6
    assert "mount_hole_span_x_mm" not in data["locked_dimensions"]["pico"]
    assert "mount_hole_span_x_mm" not in data["locked_dimensions"]["tlc5947"]
    assert data["unresolved_dimensions"]["camera_innomaker_imx296"]["board_x_mm"] is None
    assert data["unresolved_dimensions"]["pico_mount"]["mount_hole_span_x_mm"] is None
    assert data["unresolved_dimensions"]["tlc5947_mount"]["mount_hole_span_x_mm"] is None
